...
from grasp_det_seg.data_OCID.sampler import ARBatchSampler
from grasp_det_seg.data_OCID.transform import OCIDTransform
...
FORCE_CPU = False # set to True to run on CPU
LOG_DIR = "DATA/logs" # path to log directory
CONFIG = "grasp_det_seg/config/defaults/det_seg_OCID.ini" # path to configuration file
MODEL = "ckpt_files_OCID/pretrained/model_last.pth.tar" # path to model file
DATA = "DATA/OCID_grasp/data_split" # path to dataset
OUT_DIR = "DATA/output" # path to output directory
        ...
        for class_dir in os.listdir(anno_per_class_dir):
            ...
            for bbx_pred_i, cls_pred_i, obj_pred_i in zip(bbx_pred, cls_pred, obj_pred):
                if obj_pred_i.item() > threshold:
                    ...
                    r_bbox_ = OCIDTransform._Rotate2D(pts, cnt, 90 - theta)
...
def make_dataloader(config):
    ...
    log_debug("Creating dataloaders for dataset in %s", DATA)
    ...
    val_db = OCIDTestDataset(DATA, config["root_path"], config["test_set"], val_tf)
    val_sampler = ARBatchSampler(
        val_db, config.getint("val_batch_size"), False)
...
def main():
    ...
    # rank = 0, world_size = 1, device_id = 0
    ...
    log_debug("Loading snapshot from %s", MODEL)
    snapshot = resume_from_snapshot(model, MODEL, ["body", "rpn_head", "roi_head", "sem_head"])

    # Init GPU stuff
    torch.backends.cudnn.benchmark = config["general"].getboolean("cudnn_benchmark")
    if FORCE_CPU:
        model = DataParallel(model.cpu(), device_ids=[0], output_device=0)
    else:
        model = DataParallel(model.cuda(device), device_ids=[0], output_device=0)

    save_function = partial(save_prediction_image, out_dir=OUT_DIR)
